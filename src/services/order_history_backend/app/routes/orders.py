...
_next_order_id_container: list[int] = [1] # Changed to a mutable list for shared access

# Pre-populate with dummy data for testing
def _initialize_dummy_orders():
    # Only initialize if the database is empty to avoid re-adding on reload
    if not _orders_db:
        ...
        _orders_db[_next_order_id_container[0]] = { # Use _next_order_id_container[0]
            "id": _next_order_id_container[0],
            "user_id": 1,
            "items": order1_items,
        ...
        _next_order_id_container[0] += 1 # Increment the value in the list
        ...
        _orders_db[_next_order_id_container[0]] = {
            "id": _next_order_id_container[0],
            "user_id": 1,
            "items": order2_items,
